...
func display_text(text_to_display: String):
	text = text_to_display  # Corrigindo o problema
	letter_index = 0  # Evita acessar um índice inválido
	...
	text_label.text = ""  # Limpa o texto para o efeito de digitação
	display_letter()  # Começa a exibir as letras uma por uma
	

func display_letter():
	if letter_index >= text.length():
		text_display_finished.emit()
		return  # Evita acessar um índice inválido
	...
	if letter_index >= text.length():
		text_display_finished.emit()
		return  # Garante que a função não continue rodando após o fim da string

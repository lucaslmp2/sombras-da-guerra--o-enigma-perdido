...
@onready var area_porta: Area2D = get_node("../AreaSaida2") # Referência para a Area2D chamada "Porta"
...
func _on_body_entered(body: Node2D) -> void:
	...
	if body.get_collision_layer_value(player_layer):
		if area_porta and area_porta.has_node("saida2"):
			var saida_colisao = area_porta.get_node("saida2") as CollisionShape2D # Obtém a CollisionShape2D
